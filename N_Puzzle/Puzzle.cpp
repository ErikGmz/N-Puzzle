...
void Puzzle::generaPuzzle() {
	...
	do {
		...
		this->puzzle[0][contador_x][contador_y++] = random;
		if (contador_y == this->tipo_puzzle) {
			contador_y = 0;
			contador_x++;
		}
	} while (contador_y < this->tipo_puzzle - 1 && contador_x < this->tipo_puzzle - 1);
	return;
}
...
bool Puzzle::operator==(Puzzle &puzzle) {
    return this->puzzle == puzzle.puzzle;
}
...
int Puzzle::generar_numero_aleatorio() {
	switch (this->tipo_puzzle) {
	case 3: return (rand() % 9);
	case 4: return (rand() % 16);
	case 5: return (rand() % 25);
	default: return 0;
	}
}
